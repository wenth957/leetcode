...
savePath <- file.path(
  getwd(), "Feature engineering", targetDate,
  saveFn
)
...
if (loadCounts && file.exists(savePath)) {
  ...
  sortedDates <- as.Date(rownames(allCounts)) # 列按日期排序
} else {
  ...
  targetCols <- names(train)[-(1:24)] # 最后24列
  nbTargetCols <- length(targetCols) # 保留长度

  # List all unique sorted dates
  sortedDates <- sort(unique(train$fecha_dato))[-1] # 按时间排序
  nbDates <- length(sortedDates) # 时间长度
  ...
  # Calculate all monthly counts # 计算所有月份的数量
  for (i in 1:nbTargetCols) {
    targetCol <- targetCols[i]

    # Show progress message
    cat(
      "Processing target variable", i, "of", nbTargetCols, "@",
